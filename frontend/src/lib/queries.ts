...
import { RegisterInputProps, FavInputProps } from '@/typings/typings';
...
const postFavorites = async (values: FavInputProps) => {
  const res = await fetchAPI(
    '/favorites',
    {},
    {
...
export const useFavMutation = () => {
  return useMutation((values: FavInputProps) => postFavorites(values), {});
};

export const useSearchQuery = (value: any, jwt: string) => {
  return useQuery({
